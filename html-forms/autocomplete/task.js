class Autocomplete {
	...
	getMatches(text) {
		const itemOption = this.input
		let result = []

		for (let i = 0; i < itemOption.options.length; i++) {
			if (itemOption.options[i].text.includes(text) === true) {
				result.push({
					text: itemOption.options[i].text,
					value: itemOption.options[i].value,
				})
			}
		}
		return result
